...
# ---- Reference data for 3 towns (Northeast Region) ----
MRT_COORDS_ALL = {
...
# ====================== TOWN VS PRICE HELPERS ======================

def _compute_town_deviation(df: pd.DataFrame) -> pd.DataFrame:
    """Compute each town's price deviation from the 3-town average."""
    baseline = df["price_per_sqm"].mean()
    result = []
    for town_name in df["town"].unique():
        sub = df[df["town"] == town_name]
        town_avg = sub["price_per_sqm"].mean()
        deviation = (town_avg - baseline) / baseline * 100
        result.append({
            "镇区": town_name,
            "均价 (S$/sqm)": round(town_avg, 0),
            "基准 (S$/sqm)": round(baseline, 0),
            "偏离度": round(deviation, 2),
            "样本数": len(sub),
        })
    return pd.DataFrame(result).sort_values("偏离度", ascending=False)
...
def _q1_value_preservation(df: pd.DataFrame):
    ...
    st.caption("Town vs Price — 镇区偏离度分析 · 老旧 vs 新近组屋 · MRT 沿线 vs 远离 MRT · 个人推荐")
    ...
    # ----- 1. Town vs Price: Deviation Analysis -----
    st.subheader("📊 Town vs Price：镇区价格偏离度分析")

    deviation_df = _compute_town_deviation(df)

    # Bar chart: deviation from average
    colors = []
    for _, row in deviation_df.iterrows():
        if row["偏离度"] > 0:
            colors.append("#FF6B6B")  # 溢价用红色
        else:
            colors.append("#4ECDC4")  # 折价用绿色

    fig = px.bar(
        deviation_df, x="镇区", y="偏离度",
        text=deviation_df["偏离度"].apply(lambda x: f"{x:+.1f}%"),
        title="各镇区价格偏离度 (vs 三镇区均价基准)",
        color_discrete_sequence=[colors],  # doesn't work well; use go instead
    )
    # Rebuild with go for per-bar coloring
    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=deviation_df["镇区"], y=deviation_df["偏离度"],
        text=deviation_df["偏离度"].apply(lambda x: f"{x:+.1f}%"),
    ...
    st.dataframe(
        deviation_df.rename(columns={
            "偏离度": "偏离度 (%)", "均价 (S$/sqm)": "镇区均价",
            "基准 (S$/sqm)": "三镇区基准",
        }),
        width='stretch', hide_index=True,
    )

    most_expensive = deviation_df.iloc[0]
    cheapest = deviation_df.iloc[-1]
    st.caption(
        f"溢价最高: **{most_expensive['镇区']}** ({most_expensive['偏离度']:+.1f}%) | "
        f"折价最深: **{cheapest['镇区']}** ({cheapest['偏离度']:+.1f}%)"
    )

    # ----- 2. Town CAGR comparison -----
    st.subheader("📈 各镇区近5年单价涨跌幅 (CAGR)")
    ...
    if cagr_data:
        ...
        fig = px.bar(
            ...
            color="镇区",
            color_discrete_map={
                "PUNGGOL": "#4ECDC4", "SENGKANG": "#45B7D1", "HOUGANG": "#96CEB4",
            },
        )
        fig.update_traces(textposition="outside")
        fig.update_layout(height=380, margin=dict(l=0, r=0, t=30, b=0), showlegend=False)
    ...
    mrt_near = df[df["mrt_zone"] == "<500m"]
    mrt_far = df[df["mrt_zone"] == ">1km"]
    ...
    # Cross matrix: deviation × CAGR
    st.markdown("#### 偏离度 × CAGR 交叉分析矩阵")

    if cagr_data:
        cagr_df_indexed = pd.DataFrame(cagr_data).set_index("镇区")
        combined_analysis = deviation_df.copy()
        combined_analysis["CAGR"] = combined_analysis["镇区"].map(
            cagr_df_indexed["年化涨幅"]
        )
        # Classify each town
        for _, row in combined_analysis.iterrows():
            ...
            if pd.isna(cagr_val):
                continue
            if dev < 0 and cagr_val > 0.02:
                label = "🔵 潜力增长区（折价+高增长）"
            elif dev < 0 and cagr_val <= 0.02:
                label = "🟢 价格洼地（折价+低增长）"
            elif dev > 0 and cagr_val > 0.02:
                label = "🔴 热门溢价区（溢价+高增长）"
            else:
            ...
            st.metric(
                f"{row['镇区']}：{label}",
                f"偏离 {row['偏离度']:+.1f}% | CAGR {cagr_val*100:+.1f}%",
            )
    ...
    for town_name in df["town"].unique():
        for ft in df["flat_type"].unique():
            sub = df[(df["town"] == town_name) & (df["flat_type"] == ft)]
            if len(sub) < 50:
                continue
            c = _compute_cagr(sub, 2021, 2026)
            town_dev = deviation_df[deviation_df["镇区"] == town_name]["偏离度"].values
            town_dev = town_dev[0] if len(town_dev) > 0 else 0
            combo_cagr.append({
                "镇区": town_name, "房型": ft, "年化涨幅": c,
                "偏离度": town_dev, "样本": len(sub),
            })
...
基于以上 Town vs Price 分析，如果我有预算，我会优先选择 **{best['镇区']}** 的 **{best['房型']}** 组屋。以下是数据支撑：

**1. 镇区选择依据**：从偏离度来看，{best['镇区']} 相对三镇区均价偏离 {best['偏离度']:+.1f}%，{'处于折价区间，具有价格洼地优势' if best['偏离度'] < 0 else '虽有小幅溢价，但增值潜力足以覆盖' if best['偏离度'] < 5 else '溢价较高，需谨慎评估' if best['偏离度'] > 5 else ''}。同时近 5 年 CAGR 达 {best['年化涨幅']*100:+.1f}%，增值表现在三镇区中{'领先' if best['年化涨幅'] > 0.03 else '稳健' if best['年化涨幅'] > 0.01 else '一般'}。

**2. 房型选择依据**：{best['房型']} 在数据中展现出较好的保值能力，5 年年化涨幅达 {best['年化涨幅']*100:+.1f}%。该房型在转售市场上需求稳定、流动性好。

**3. 新旧对比考量**：{'新近组屋' if new_cagr > old_cagr else '老旧组屋'} 的保值能力更强（{new_cagr*100:+.1f}% vs {old_cagr*100:+.1f}%），{'因此倾向于选择剩余租约较长的房产。' if new_cagr > old_cagr else '老旧组屋虽然租约较短，但地段配套成熟，具有一定的抗跌性。'}

**4. MRT 距离考量**：MRT 沿线 (<500m) 组屋与远离 MRT (>1km) 组屋年化涨幅 {'更高' if near_cagr > far_cagr else '相近'}（{near_cagr*100:+.1f}% vs {far_cagr*100:+.1f}%），靠近地铁站仍是重要的保值因素。

**5. 综合策略**：综合偏离度 + CAGR 交叉分析，最优策略是选择**折价或接近均值的镇区中靠近 MRT 且租约较长的组屋**——既能享受价格洼地的增长红利，又具备交通便利性和较长的资产寿命。
        """)
...
def _q2_strategy_rationale(df: pd.DataFrame):
    ...
    strat = st.radio(
        "选择策略",
        ["镇区均价偏离策略", "长租约保值策略", "MRT 便利策略", "低总价入门策略"],
        horizontal=True, key="q2_strategy",
    )

    if strat == "镇区均价偏离策略":
        desc = "选择价格低于三镇区均价的房源，靠近 MRT 的 4-Room/5-Room，利用价格向均值回归获取增值。"
        budget_max = 600_000
        types = ["4-Room", "5-Room"]
...
{'值得注意的' if cagr_diff < 0.01 or cv_diff > 0.02 else ''}一个反直觉现象是：{'虽然市场普遍认为价格偏低的镇区增值潜力更大，但验证期数据显示部分价格偏高的镇区在波动性上更具优势——这说明高价镇区的需求刚性更强，价格不易受外部冲击影响。' if '偏离' in name else ''}{'虽然 MRT 沿线通常被认为具有溢价优势，但数据表明溢价主要体现在总价而非涨幅上——换言之，买 MRT 旁更贵，但涨幅不一定更高。' if 'MRT' in name else ''}{'长租约房产虽然折旧风险低，但由于定价中已包含租约溢价，其「超额涨幅」并不明显，未来的增值空间已被当前较高的买入价部分透支。' if '租约' in name else ''}{'低总价入门策略虽然门槛低、流动性好，但低价房源往往存在面积小、房龄老、楼层低等不利因素，验证期内涨幅可能不及中等价位的房源。' if '入门' in name else ''}
